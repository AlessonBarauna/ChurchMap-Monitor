...
export class MapComponent implements AfterViewInit, OnDestroy {
  ...
  constructor() {
    // Re-renderiza marcadores quando a lista muda
    effect(() => {
    ...
    // Foca e abre popup da igreja selecionada
    effect(() => {
  ...
  private renderMarkers(churches: Church[], diff: DiffResult | null): void {
    ...
    for (const c of churches) {
      ...
      marker.bindPopup(`
        <div style="padding:1rem;min-width:180px;font-family:'Syne',sans-serif">
          ...
          <div style="font-size:0.7rem;color:#5a5680;font-family:'DM Mono',monospace">ID: ${c.id}</div>
          <a href="https://www.openstreetmap.org/node/${c.id}" target="_blank"
             style="display:inline-flex;align-items:center;gap:4px;margin-top:0.5rem;font-size:0.75rem;color:#7c6fff;text-decoration:none;font-weight:600">
            Ver no OSM →
          </a>
        </div>
